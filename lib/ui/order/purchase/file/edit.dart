...
import 'package:tob/ui/user/order/file/file_common.dart';
...
class _FileEditPageState extends State<FileEditPage> {
  ///是否需要在加载状态
  bool _loading = true;
  PurchaseFileEntity _purchaseFileEntity;
  ...
  List<PurchaseFileImageEntity> _fileImageList = [];
  ...
  //获取资料详情
  void getData() {
    Http.getInstance().purchaseFile(vid: widget.vid).then((value) {
      setState(() {
        _purchaseFileEntity = value;
        if (_purchaseFileEntity != null) {
          ...
          if(typeIndex ==1){
            ...
            _businessImage = value.businessImage;
  ...
  void getFileImage() {
    Http.getInstance()
        .getPurchaseFileImageField(
            pid: int.parse(widget.pid), vinId: widget.vid)
        .then((value) {
      if (mounted) {
  ...
  //检查api是否已经请求完
  void checkApiEnd() {
    if (_profile == true && _fileImage == true) {
      _loading = false;
      setState(() {});
    }
  }
                ...
                child: Center(
                  child: Column(
                    children: [
                      ...
                      //提交
                      Offstage(
                        offstage: _purchaseFileEntity == null ||
                                (_purchaseFileEntity.state != 2 &&
                                    _purchaseFileEntity.state != 3)
                            ? false
                            : true,
                        child: eleButton(
                            func: () {
                              ...
                              for (int i = 0; i < _fileImageList.length; i++) {
                                PurchaseFileImageEntity value =
                                    _fileImageList[i];
                                if (value.imgs == "") {
                                  Loading.toast(
                                      msg: value.fieldName + "：至少传一张图片");
                                  havingImg = false;
                                  break;
                                }
                              }
                              ...
                              //验证行驶证反面
                              if (_licenseFImage == null ||
                                  _licenseFImage.length <= 0) {
                                KlAlert.showAlert(
                                    content: '请上传行驶证反面',
                                    sureFunc: () {
                                      MyRoute.router.pop(context);
                              ...
                              if (typeIndex == 1) {
                                //身份证正面
                                if (_idCardZImage == null ||
                                    _idCardZImage.length <= 0) {
                                  KlAlert.showAlert(
                                      content: '请上传客户身份证正面',
                                      sureFunc: () {
                                        MyRoute.router.pop(context);
                                ...
                                //身份证正面
                                if (_idCardFImage == null ||
                                    _idCardFImage.length <= 0) {
                                  KlAlert.showAlert(
                                      content: '请上传客户身份证反面面',
                                      sureFunc: () {
                                        MyRoute.router.pop(context);
                              ...
                              KlAlert.showAlert(
                                  ...
                                  sureFunc: () {
                                    //在这里处理自定义的资料图片
                                    List<PurchaseFileImageEntity>
                                        carFileImageList = [];
                                    carFileImageList = _fileImageList;
                                    carFileImageList
                                        .asMap()
                                        .forEach((key, value) {
                                      carFileImageList[key].imgs =
                                          CommonUtil.imgDeal(
                                              imgStr: value.imgs,
                                              type: Config.dynamic);
                                    });
                                    ...
                                    Http.getInstance()
                                        .purchaseFileUpload(
                                            ...
                                            vFUrl: CommonUtil.imgDeal(
                                                imgStr: _licenseZImage,
                                                type: Config.vehicle),
                                            vBUrl: CommonUtil.imgDeal(
                                                imgStr: _licenseFImage,
                                                type: Config.vehicle),
                                            cUrl: CommonUtil.imgDeal(
                                                imgStr: _certImage,
                                                type: Config.carCert),
                                            iUrl: CommonUtil.imgDeal(
                                                imgStr: _invoiceImage,
                                                type: Config.carInvoice),
                                            aid: _purchaseFileEntity?.id,
                                            certUrl: CommonUtil.imgDeal(
                                                imgStr: _certificateImage,
                                                type: Config.carCert),
                                            idCardFUrl: _idCardZImage != null && _idCardZImage.length>0?CommonUtil.imgDeal(
                                                imgStr: _idCardZImage,
                                                type: Config.idCard):'',
                                            idCardBUrl:_idCardFImage!=null&& _idCardFImage.length>0? CommonUtil.imgDeal(
                                                imgStr: _idCardFImage,
                                                type: Config.idCard):'',
                                            code: _idCardController.text,
                                            businessCode:
                                                _idCardController.text,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Offstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.clientMobileState == '3'
                ? false
                : true,
            child: Container(
              width: 690.w,
              child: Text(
                '${_purchaseFileEntity?.clientMobileRemarks}',
                style: TextStyle(
  ...
  top() {
    return Column(
      children: [
        ...
        conFileContainer(
          title: '颜色',
          wid: '${widget.colorName != null ? widget.colorName : ''}',
          widget: false,
        ),
        if (_purchaseFileEntity?.state != 2 && _purchaseFileEntity?.state != 3)
          conFileContainer(
              title: '客户类型',
              wid: Row(
        ...
        onTap: () {
          apiUploadImg(
              ...
              func: (img) {
                ...
                Http.getInstance()
                    .ocr(image: img, type: 'front', imgType: 'vehicle')
                    .then((ocrRes) {
  ...
  licenseF() {
    return Column(
      children: [
            ...
            onTap: () {
              apiUploadImg(
                ...
                func: (img) {
                  Loading.show(status: '行驶证认证中...');
                  Http.getInstance()
                      .ocr(image: img, type: 'back', imgType: 'vehicle')
                      .then((ocrRes) {
                    setState(() {
                      _licenseFImage = img;
                    });
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.licenseFState == 3
                ? false
                : true,
            errorText: _purchaseFileEntity?.licenseFRemarks),
      ],
    );
  }
  ...
  certificateImage() {
    return Column(
      children: [
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.certificateImageState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.certificateImageRemarks),
      ],
    );
  }
  ...
  cert() {
    return Column(
      children: [
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.certState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.certRemarks),
      ],
    );
  }
  ...
  invoice() {
    return Column(
      children: [
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.invoiceState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.invoiceRemarks),
      ],
    );
  }
  ...
  idCardZ() {
    return Column(
      children: [
                  ...
                  Http.getInstance()
                    ...
                    setState(() {
                      if (ocrRes.data['name'] != _liName) {
                        KlAlert.showAlert(
                            content:
                                '身份证和行驶证的信息不匹配,请重新上传，请确保行驶证与身份证绝对清晰，无反光、模糊等情况',
                            sureFunc: () {
                              MyRoute.router.pop(context);
                            });
                        return false;
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.clientIdZImageState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.clientIdZImageRemarks),
      ],
    );
  }
  ...
  idCardF() {
    return Column(
      children: [
            ...
            onTap: () {
              ...
              apiUploadImg(
                  ...
                  func: (img) {
                    Loading.show(status: '身份证认证中...');
                    Http.getInstance()
                        .ocr(image: img, type: 'back', imgType: 'idcard')
                        .then((ocrRes) {
                      setState(() {
                        _idCardFImage = img;
                      });
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.clientIdFImageState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.clientIdFImageRemarks),
      ],
    );
  }
  ...
  business() {
    return Column(
      children: [
            ...
            onTap: () {
              ...
              apiUploadImg(
                  ...
                  func: (img) {
                    ...
                    Http.getInstance()
                        .ocr(
                            image: img, imgType: Config.business, type: 'front')
                        .then((idCardRes) {
                      if (idCardRes.data['name'] != _liName) {
                        KlAlert.showAlert(
                            content:
                                '营业执照和行驶证的信息不匹配，请重新上传。请确保行驶证与营业执照绝对清晰，无反光、模糊等情况。',
                            sureFunc: () {
                              MyRoute.router.pop(context);
                            });
                        return false;
        ...
        comOffstage(
            offstage: _purchaseFileEntity != null &&
                    _purchaseFileEntity.businessImageState == '3'
                ? false
                : true,
            errorText: _purchaseFileEntity?.businessImageRemarks),
      ],
    );
  }
  ...
  //资料图片视图
  fileImageWidget() {
    comThisContainer(PurchaseFileImageChild carFileImageChild, int index) {
            ...
            children: [
              for (int i = 0; i < imgArr.length; i++)
                GestureDetector(
                  onTap: () {
                    imgPreview(imgArr, i,
                        title: _fileImageList[index].fieldName);
                  },
                  child: Container(
                    width: _imgW,
                    height: _imgH,
  ...
  Container conWidgetContainer(String title, Widget wid) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(width: 380.w, height: 90.w, child: wid),
        ],
      ),
    );
  }
